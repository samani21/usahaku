    ...
    return (
        <div className="space-y-4">
            <div className={`sticky bottom-6 ${isDarkMode ? "bg-black/90 border-slate-700" : "bg-white/90 border-slate-200"} backdrop-blur-md p-2 pl-6 rounded-full shadow-xl border  flex items-center justify-between`}>
                ...
                <HandleCheckout isBuild={isBuild} className={`bg-[var(--summary-primary-color)]/80 text-white p-4 rounded-full hover:bg-[var(--summary-primary-color)] transition-all shadow-lg`}>
                    <ArrowRight size={20} />
                </HandleCheckout>
            </div>
        </div>
