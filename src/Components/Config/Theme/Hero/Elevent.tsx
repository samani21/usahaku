...
const Elevent = ({ isDarkMode, headline, subHeadline, ctaText, imageHero }: Props) => {
    return (
        <section>
            <div className={`p-8 rounded-[3rem] transition-all flex flex-col md:flex-row items-center gap-8 ${isDarkMode ? 'bg-slate-900 shadow-inner' : `shadow-sm bg-[var(--hero-primary-color)]/5`}`}>
                ...
                <div className="flex-1 text-center md:text-left">
                    <h2 className={`text-4xl md:text-5xl font-black ${isDarkMode ? 'text-[var(--hero-primary-color)]' : 'text-[var(--hero-primary-color)]'} mb-4`}> {headline}</h2>
                    <p className={` ${isDarkMode ? 'text-[var(--hero-primary-color)]' : 'text-[var(--hero-primary-color)]'} text-lg mb-8`}>{subHeadline}</p>
