...
const One = ({ categories, isDarkMode, onClick }: Props) => {
    ...
    return (
        <section className="py-12 px-2 sm:px-6 max-w-7xl mx-auto">
            <div className={`grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-6 ${isDarkMode ? "text-white" : "text-slate-900"}`}>
                <div onClick={() => {
                    onClick && onClick(null)
                    handleScroll()
                }} className={`relative group overflow-hidden rounded-[2rem] h-40 sm:h-64 cursor-pointer shadow-xl ${isDarkMode ? "shadow-black/20" : "shadow-slate-200/50 "}`}>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent group-hover:via-black/20 transition-colors p-8 flex flex-col justify-end">
                        <div className="transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                            <h3 className="text-sm sm:text-2xl font-black text-white uppercase tracking-tight italic">
                                Semua
                            </h3>
                ...
                {categories.map((cat, i) => (
                    <div onClick={() => {
                        ...
                        handleScroll()
                    }} key={i} className={`relative group overflow-hidden rounded-[2rem] h-40 sm:h-64 cursor-pointer shadow-xl ${isDarkMode ? "shadow-black/20" : "shadow-slate-200/50 "}`}>
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent group-hover:via-black/20 transition-colors p-8 flex flex-col justify-end">
                            <div className="transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                                ...
                                <div className="flex items-center gap-3 mt-1">
                                    ...
                                    <div className="hidden sm:block h-px w-0 group-hover:w-12 bg-white/50 transition-all duration-500" />
                                </div>
                            </div>
                        </div>

                        {/* Corner Decorative Icon */}
                        <div className="hidden sm:block absolute top-6 right-6 p-3 rounded-full bg-white/10 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-500 border border-white/20">
                            <ChevronRight className="text-white w-4 h-4" />
                        </div>
                    </div>
                ))}
